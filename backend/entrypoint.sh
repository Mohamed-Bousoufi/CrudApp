...
until python -c "
import MySQLdb
MySQLdb.connect(host='$DB_HOST', user='$DB_USER', passwd='$DB_PASS', db='$DB_NAME')
" 2>/dev/null; do
    counter=$((counter + 1))
...
users = [
    {'first_name': 'John', 'last_name': 'Doe', 'email': 'john@example.com', 'gernder': 'Male'},
    {'first_name': 'Jane', 'last_name': 'Smith', 'email': 'jane@example.com', 'gernder': 'Female'},
    {'first_name': 'Bob', 'last_name': 'Wilson', 'email': 'bob@example.com', 'gernder': 'Male'},
]
for u in users:
